...
def trunc_normal_(tensor, mean=0., std=1., a=-2., b=2.):
    ...
    with jt.no_grad():
        l = norm_cdf(((a - mean) / std))
        u = norm_cdf(((b - mean) / std))
        low, high = 2 * l - 1, 2 * u - 1
        tensor.assign(jt.rand(tensor.shape, tensor.dtype) * (high - low) + low)
        tensor.assign(jt.erfinv(tensor))
        tensor.assign(tensor * (std * math.sqrt(2.0)))
        tensor.assign(tensor + mean)
        tensor.assign(jt.clamp(tensor, a, b))
        return tensor
...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=1000, depths=[3, 3, 9, 3], dims=[96, 192, 384, 768],
                 return_stages=["layer1", "layer2", "layer3", "layer4"], drop_path_rate=0.0,
                 layer_scale_init_value=1e-06, norm_eval=True, frozen_stages=-1, head_init_scale=1.0):
        super().__init__()
        self.downsample_layers = nn.ModuleList()
        stem = nn.Sequential(nn.Conv(in_chans, dims[0], 4, stride=4),
                             LayerNorm(dims[0], eps=1e-06, data_format='channels_first'))
        self.downsample_layers.append(stem)
        for i in range(3):
            downsample_layer = nn.Sequential(LayerNorm(dims[i], eps=1e-06, data_format='channels_first'),
                                             nn.Conv(dims[i], dims[(i + 1)], 2, stride=2))
